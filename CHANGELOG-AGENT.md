...
## 2026-09-01 — Hábitos: eliminar/reactivar por día reflejado en la vista semanal
- **Qué:** en el DayView cada hábito tiene un botón "×" que lo deshabilita solo para ese día (la fila queda atenuada con el nombre tachado y un botón "↺" para reactivarlo). En la WeekView, ese día aparece como celda deshabilitada (borde punteado, "–"); al hacer clic sobre ella se reactiva. El hábito nunca se borra globalmente, solo se desactiva por día.
- **Por qué:** el usuario quería poder eliminar un hábito de un día concreto, verlo deshabilitado en la vista semanal y poder volver a activarlo.
- **Archivos:** src/views/DayView.jsx (handlers `disableHabitDay`/`enableHabitDay` y render de hábitos con botones ×/↺), src/views/DayView.css (`.todo__item--habit-off`, `.todo__check--disabled`, `.todo__text--off`, `.todo__habit-remove`, `.todo__habit-restore`), src/views/WeekView.jsx (`toggle` reactiva si el día está off; celdas móvil y desktop con estado deshabilitado), src/views/WeekView.css (`.habits__check--disabled`).
- **Notas:** el estado deshabilitado se guarda en el mismo store `checks-YYYY-Wnn` con claves `off-<habitId>-<díaIndex>=true`, sin colisionar con las claves de check (`<habitId>-<díaIndex>`), por lo que se respalda en Firestore automáticamente sin nuevos prefijos. Al deshabilitar se limpia el check de ese día. Estilos con variables de tema (`--color-surface-hover`, `--color-border`, `--color-text-muted`, `--todo-check`, `--color-accent-red`), válidas en ambos temas.

## 2026-09-01 — Notas: cuadro con viñeta, fix de check al agregar y TODO a alto completo
- **Qué:** en el DayView, al agregar/editar una nota se muestra un cuadro gris claro con un punto (viñeta) al centro; al hacer click se quita el punto y el cuadro se ilumina (borde amarillo) indicando que será un check, y con otro click vuelve al estado inicial. Se corrigió el bug por el que no se podía crear una nota **con** check: el click en el cuadro robaba el foco del input y disparaba `onBlur` → `commitAdd`/`commitEdit`, guardando la nota como "sin check" antes de togglear. Las notas guardadas muestran un check funcional (si tienen check) o solo la viñeta; al editarlas vuelve el cuadro toggle. Además, en desktop el TODO (columna 2) ocupa el alto disponible con su lista scrolleable.
- **Por qué:** el usuario necesitaba poder agregar un item con o sin check, y que el toggle fuera claro y aprovechara el alto de la columna.
- **Archivos:** src/views/DayView.jsx (render de Notas + `onMouseDown preventDefault` en los cuadros), src/views/DayView.css (`.note-item__box`, `.note-item__check`, `.note-item__bullet`; `.day-view__top-row`, `.todo`, `.todo__list`, `.day-view__col--side-left`).
- **Notas:** `hasCheck` persiste en localStorage. El cuadro/check usan `--accent-yellow` (temático). La columna 2 pasa a `overflow: hidden` y el TODO crece con `flex:1` mientras su lista scrollea internamente; el layout móvil no se afecta.
